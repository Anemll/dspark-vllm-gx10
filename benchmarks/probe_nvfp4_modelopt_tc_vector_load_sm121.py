...
"""Run the real-layer C/C finite-E8M0 gate with vector shared loads."""
...
from scripts import patch_b12x_w4a16_modelopt_vector_load as patcher
...
def run(args: argparse.Namespace) -> int:
    ...
    vector_load_passed = bool(candidate_events) and all(
        event["modelopt_vector_load"] for event in candidate_events
    )
    ...
    if not vector_load_passed:
        failures.append({"kind": "modelopt_vector_load_specialization"})
    ...
    report["modelopt_vector_load_probe"] = {
        ...
        "vector_load_specialization_passed": vector_load_passed,
        "source_pin_passed": source_pin_passed,
        "finite_scale_contract": finite_scale_contract,
        "shared_load_instructions_per_packed_word_before": 4,
        "shared_load_instructions_per_packed_word_after": 2,
        "source_n_rotation_before": "per_16_byte_copy",
        "source_n_rotation_after": "once_per_staged_k_tile",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--modelopt-vector-load", action="store_true", required=True)
    return parser
