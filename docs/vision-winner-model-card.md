...
All language and multimodal model tensors come from **DeepSeek AI's official
Vision-Exp checkpoint**. This includes the language model, vision tower,
vision aligner, image sentinels and routing data, tokenizer, and trained DSpark
draft layers. No model merge, projector transplant, fine-tuning, or new weight
quantization was applied to the accepted checkpoint.
...
0731, W4A4, or an NVIDIA NVFP4 model. The serving profile's `fp8` setting is
the **KV-cache format**, not the source weight format.

## Credits

| Component | Credit and source | Role in this package |
|---|---|---|
| Model, tokenizer, vision tower/aligner and DSpark draft | [DeepSeek AI — DeepSeek-V4-Flash-Vision-Exp](https://huggingface.co/deepseek-ai/DeepSeek-V4-Flash-Vision-Exp) | Complete, unmodified source checkpoint |
...
> DeepSeek-V4-Flash-Vision-Exp checkpoint, using vLLM, B12X and DSpark. Model
> and vision weights are from DeepSeek AI. NVIDIA provides the DGX Spark/GB10
> and CUDA platform; no NVIDIA 0731 NVFP4 model weights are included.
