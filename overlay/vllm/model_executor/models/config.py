...
class DeepseekV4ForCausalLMConfig(VerifyAndUpdateConfig):
    ...
    def verify_and_update_config(vllm_config: "VllmConfig") -> None:
        ...
        if getattr(config, "architectures", None) != ["DeepseekV4ForConditionalGeneration"]:
            return
        if not vllm_config.use_v2_model_runner:
            raise ValueError("This Spark vision backport requires the V2 model runner")
        if vllm_config.scheduler_config.max_num_batched_tokens < config.vision_max_n_token + 3:
            raise ValueError("Batch token budget cannot hold one complete DSv4 image")
