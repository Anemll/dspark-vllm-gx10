            ...
            if self.model is not None:
                ...
                if n_predict is not None:
                    if self.num_speculative_tokens is None:
                        ...
                        and not (
                            self.method == "dspark"
                            and self.draft_model_config.architectures
                            == ["DSparkDraftModel"]
                        )
                    ):
                        # MTP reuses modules per step. DeepSeek-V4 DSpark
                        # instead stacks its draft layers for one block pass;
                        # its block length need not be a multiple of n_predict.
                        raise ValueError(
