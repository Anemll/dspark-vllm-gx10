...
def _pool_bytes_per_block(
    ...
    if _use_packed_kv_cache_config(vllm_config, kv_cache_groups):
        ...
        return _align_packed_block_stride(
            kv_cache_groups,
...
def _align_packed_block_stride(
    kv_cache_groups: list[KVCacheGroupSpec],
    block_stride: int,
) -> int:
    """Round a packed block stride up to what its layers require.

    A layer whose page is reached through a strided view over the shared slab sees
    the slab's block stride as that view's outer stride, so a kernel that requires an
    aligned stride has to be honoured here.  Under the DSpark NVFP4 layout every
    DeepSeek-V4 page is a multiple of 64 bytes except the C128 main page
    (2 * 584 = 1168, congruent to 16), so without this the stride is aligned only
    when a worker happens to own a multiple of four C128 layers -- true with every
    layer resident by accident, false for most pipeline-parallel splits.

    Both callers that plan the packed layout go through this. Note that
    ``_max_memory_usage_bytes_from_groups`` still computes bytes per block on its own
    for the admission check, so its estimate is short by the padding.

    This aligns the stride, not the base offset of each page within a block.
    """
    stride_alignment = 1
    for group in kv_cache_groups:
        spec = group.kv_cache_spec
        for layer_name in group.layer_names:
            ...
            if isinstance(layer_spec, AttentionSpec):
                alignment = layer_spec.block_stride_alignment_bytes
                if alignment <= 0:
                    raise ValueError(
                        f"Layer {layer_name}: block_stride_alignment_bytes must "
                        f"be positive, got {alignment}"
                    )
                stride_alignment = math.lcm(stride_alignment, alignment)
    return round_up(block_stride, stride_alignment)
...
def _get_kv_cache_config_packed(
    ...
    total_num_bytes_per_block = _align_packed_block_stride(
        kv_cache_groups, total_num_bytes_per_block
    )
        ...
        for layer_name, spec in kv_cache_spec.items():
            if isinstance(spec, SlidingWindowMLASpec):
                kv_cache_spec[layer_name] = MLAAttentionSpec(
                    ...
                    block_stride_alignment_bytes=spec.block_stride_alignment_bytes,
