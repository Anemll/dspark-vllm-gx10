...
class SharedOffloadRegion:
    ...
    def __init__(
        ...
        # Unlink the mmap file immediately after mapping. The mapping stays
        # valid via the open fd, and no orphaned named file remains in /dev/shm
        # on ANY exit path (graceful OR crash/OOM/SIGKILL). instance_id churns
        # on every engine start, so without this a stale vllm_offload_*.mmap
        # file would otherwise leak on every abrupt exit. On DGX Spark (GB10
        # unified memory) these accumulate in the 128 GiB /dev/shm and can
        # push cold-start peak over the limit, tripping the kernel OOM-killer.
