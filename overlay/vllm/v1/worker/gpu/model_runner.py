...
class GPUModelRunner(LoRAModelRunnerMixin):
    def __init__(self, vllm_config: VllmConfig, device: torch.device):
        ...
        # The target-route diagnostic is imported and constructed only when
        # explicitly enabled. The normal serving path does not bind router
        ...
        if os.getenv("DSPARK_TARGET_ROUTE_CAPTURE", "0") != "0":
            ...
            self._target_route_capture_config = capture_config
    ...
    def load_model(self, load_dummy_weights: bool = False, *args, **kwargs) -> None:
        ...
        if self._target_route_capture_config is not None:
            ...
            logger.info(
                "Enabled bounded target-only route capture: rank=%d/%d steps=%d "
                "warmup=%d output=%s",
    ...
    def execute_model(
        ...
        if self.target_route_capture is not None:
            capture_target_routes = self.target_route_capture.begin_step(
