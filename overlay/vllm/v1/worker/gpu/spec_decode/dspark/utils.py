...
def load_dspark_model(target_model: nn.Module, vllm_config: VllmConfig) -> nn.Module:
    ...
    # V2 loads DSpark here, bypassing LLMBaseProposer's kernel overrides.
    # Mixed NVIDIA checkpoints need NVFP4 target and MXFP4 draft backends.
    # Preserve the target config and the existing no-override text path.
