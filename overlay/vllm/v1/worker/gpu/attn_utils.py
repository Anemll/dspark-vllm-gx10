    ...
    for req_idx, req_id in enumerate(req_ids):
        ...
        for mm_feature in mm_features.get(req_id, ()):
            ...
            if span_leading_pad_modulus:
                pad = span_leading_pad_modulus - 1 - pos.offset % span_leading_pad_modulus
                ranges = [(pos.offset + pad, pos.offset + pos.length - 1)]
            else:
                ranges = pos.extract_embeds_range()
            for r in ranges:
                if not clamps_in_kernel and sliding_window is not None and (r[1] - r[0] + 1) > sliding_window:
                    continue
