        ...
        if is_start:
            ...
            if self.profiler is None:
                ...
                if profiler_type == "torch":
                    # GB10 overlay: default to CUDA-only profiling. The CPU
                    # profiler's event buffers grow unboundedly for the life of
                    # a capture; on unified-memory GB10 that bloat competes
                    # directly with model weights and KV cache and can OOM the
                    # node. Opt back in with
                    # VLLM_TORCH_PROFILER_ACTIVITIES=CPU,CUDA (upstream default).
                    activities = [
                        a.strip()
                        for a in os.environ.get(
                            "VLLM_TORCH_PROFILER_ACTIVITIES", "CUDA"
                        ).split(",")
                        if a.strip()
                    ]
