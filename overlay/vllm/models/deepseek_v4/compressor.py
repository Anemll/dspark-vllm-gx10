...
class CompressorStateCache(torch.nn.Module, AttentionLayerBase):
    ...
    def get_kv_cache_spec(self, vllm_config: VllmConfig) -> KVCacheSpec:
        ...
        return SlidingWindowMLASpec(  # only has one vector instead of K + V
            ...
            # The CuTeDSL compressor kernels declare state_cache with
            # stride=(sym_int64(divisibility=16), sym_int64(divisibility=16), 1).
            # state_cache is float32 and is a strided view over the packed slab, so
            # its outer stride is the block stride in bytes divided by 4: the block
            # stride must be a multiple of 16 * 4 = 64 bytes.
            block_stride_alignment_bytes=64,
        )
