...
def _local_mtp_safetensors_patterns(model_path: str) -> list[str] | None:
    """Return only shards holding DSpark MTP weights for a local checkpoint.

    The target model and DSpark draft share a checkpoint. The generic loader
    drops target tensors only after opening every safetensors shard, which
    turns a three-shard DSpark load into a second full checkpoint scan on
    network storage. The supported ``allow_patterns_overrides`` model hook
    lets the draft narrow its own source before the iterator opens anything.

    This is deliberately fail-open: hub models, unindexed checkpoints, and
    malformed indexes retain upstream's complete-file behavior rather than
    risking a partial draft load.
    """
    ...
    try:
        ...
        logger.warning_once(
            "DSpark could not read local safetensors index %s; loading all "
            "checkpoint shards: %s",
            index_path,
            error,
        )
...
class DSparkDeepseekV4ForCausalLM(nn.Module):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = "") -> None:
        ...
        # DefaultModelLoader reads this hook after constructing the model. For
        # this local, indexed target checkpoint it avoids reopening all target
        # shards merely to discard their non-MTP names in ``load_weights``.
        self.allow_patterns_overrides = _local_mtp_safetensors_patterns(
