...
class DSparkConfidenceHead(nn.Module):
    """FP32 confidence projection from backbone and Markov hidden states."""
...
class DSparkDeepseekV4ForCausalLM(nn.Module):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = "") -> None:
        ...
        self.confidence_head_loaded = False
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        self._finalize_moe()
        required_confidence_param = "model.confidence_head.proj.weight"
        self.confidence_head_loaded = (
            self.confidence_head_loaded
            or required_confidence_param in loaded_params
        )
        if not self.confidence_head_loaded:
            raise RuntimeError(
                "DSpark confidence head weight was not loaded: "
                f"{required_confidence_param}"
            )
        logger.info_once(
            "DSpark draft model loaded: %d params; confidence_head_loaded=true",
            len(loaded_params),
        )
