...
class DeepseekV4MoE(nn.Module):
    def __init__(
        ...
        # Vision target and draft layers carry an additional routing bias.
        # Keep text-only construction and its compiled router byte-for-byte intact.
        if getattr(config, "vision_n_layers", 0) > 0:
            from .vision_router import install_vision_router

            install_vision_router(self)
