...
class DeepseekV4VLMultiModalProcessor(
    ...
    def _apply_prompt_updates(
        ...
        # Use the pinned processor API. Cache pad-free image transforms, then
        # insert alignment pads at final prompt offsets (including earlier images).
        expanded, placeholders = super()._apply_prompt_updates(
            token_ids, mm_prompt_updates
        )
        return align_image_placeholders(expanded, placeholders)


def align_image_placeholders(
    token_ids: list[int],
    placeholders: Mapping[str, list[PlaceholderFeaturesInfo]],
) -> tuple[list[int], dict[str, list[PlaceholderFeaturesInfo]]]:
    out: list[int] = []
    aligned = {modality: [] for modality in placeholders}
    previous_end = 0
    for item in sorted(
        (item for items in placeholders.values() for item in items),
        key=lambda item: item.start_idx,
    ):
        if item.modality != "image" or item.start_idx < previous_end:
            raise ValueError("DSv4 supports non-overlapping image placeholders only")
        out.extend(token_ids[previous_end:item.start_idx])
        offset = len(out)
        pad = COMPRESS_PAD_TO - 1 - offset % COMPRESS_PAD_TO
        tokens = [IMAGE_SENTINEL_BASE_ID + IMAGE_PAD] * pad + item.tokens
        is_embed = torch.tensor(tokens) == IMAGE_SENTINEL_BASE_ID + IMAGE
        aligned[item.modality].append(PlaceholderFeaturesInfo(
            modality=item.modality, item_idx=item.item_idx, start_idx=offset,
            tokens=tokens, is_embed=is_embed,
        ))
        out.extend(tokens)
        previous_end = item.start_idx + item.length
    out.extend(token_ids[previous_end:])
    return out, aligned
