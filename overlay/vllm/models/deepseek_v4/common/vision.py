...
def _vision_uses_data_parallel(config) -> bool:
    # The pinned vLLM helper takes no head-count argument (newer main does).
    return is_vit_use_data_parallel() or (
        config.vision_n_heads % get_tensor_model_parallel_world_size() != 0
    )
...
class DeepseekV4VisionAttention(nn.Module):
    def __init__(self, config, prefix: str = ""):
        ...
        use_data_parallel = _vision_uses_data_parallel(config)
        self.tp_size = (
